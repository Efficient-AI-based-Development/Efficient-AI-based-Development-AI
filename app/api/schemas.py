...
class ProjectMetadata(BaseModel):
    ...
    ai_model: str = Field(
        ..., description="사용할 AI 모델 (예: GPT-4, Claude, Solar-Pro)"
    )
    tech_stack: List[str] = Field(
        ..., min_length=6, max_length=6, description="기술 스택 6개"
    )
...
class PMAgentChatInput(BaseModel):
    current_metadata: ProjectMetadata = Field(
        ..., description="현재 프로젝트 메타데이터"
    )
